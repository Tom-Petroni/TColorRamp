...
try:
    from TColorRamp._consts import MENU_NAME, NODE_CLASS_NAME, PLUGIN_LOADED_ENV_VAR
    from TColorRamp._plugin_loader import ensure_node_class_loaded
except Exception:
...
def create_node() -> None:
    ...
    try:
        ...
        nuke.tprint(
            "[TColorRamp] Unable to create node '{}': {}".format(
                NODE_CLASS_NAME,
                error,
            ),
        )
...
def _create_menu() -> None:
    ...
    if toolbar.findItem(command_path) is None:
        callback = "import {} as _tcr_menu; _tcr_menu.create_node()".format(__name__)
        menu.addCommand(NODE_CLASS_NAME, callback)
...
def add_menu() -> None:
    ...
    if os.getenv(PLUGIN_LOADED_ENV_VAR) != "1":
        logger.warning("TColorRamp menu created, but plugin binary is not loaded yet.")
