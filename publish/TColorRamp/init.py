"""Main entry point for TColorRamp Nuke plugin."""
...
try:
    from TColorRamp._plugin_loader import add_plugin_path_safe
except Exception:
    from _plugin_loader import add_plugin_path_safe

logger = logging.getLogger(__name__)

_HOOKS_ENV_VAR = "TCOLORRAMP_SCRIPT_HOOKS_DONE"


def _refresh_plugin_path() -> None:
    loaded = add_plugin_path_safe()
    if not loaded:
        nuke.tprint("[TColorRamp] Plugin binary not loaded yet.")
...
try:
    ...
    logger.exception("Unexpected failure while initializing the TColorRamp plugin.")
