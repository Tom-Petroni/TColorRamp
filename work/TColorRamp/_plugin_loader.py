...
try:
    from TColorRamp._consts import (
        INSTALLATION_PATH,
        NODE_CLASS_NAME,
...
def ensure_node_class_loaded() -> str:
    ...
    if not os.path.isdir(plugin_path):
        raise PluginNotFoundError(
            (
                "TColorRamp is installed, but this Nuke version '{}' is not available "
                "in this package."
    ...
    if not os.path.isfile(binary_path):
        raise PluginNotFoundError(
            "TColorRamp binary was not found at '{}'".format(binary_path),
        )
...
def add_plugin_path_safe() -> bool:
    ...
    try:
        ...
        logger.info("TColorRamp plugin loaded successfully from '%s'.", plugin_path)
        return True
    except (PluginNotFoundError, UnsupportedSystemError, PluginLoadError):
        logger.exception("TColorRamp plugin loading failed.")
        return False
